...
public class GenerateCodeService {
	...
	public void generateCode(CGConditions cgCondition) {
		...
		for(String tableName : tableList) {
			...
			if(tableName.indexOf("oms_task_deal_rule") > -1) {
				tableNameDeal = StringUtil.removePrefix(tableName, "");
			} else {
			...
			CodeModel frontBtnFactory = codeFactoryProducer.getFactory("frontBtn");
			frontBtnFactory.generateCode(frontModelParams);
